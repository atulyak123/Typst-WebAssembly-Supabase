...
export default function Login() {
  ...
  const getMessageClasses = () => {
    const baseClasses =
      "text-center text-sm rounded-lg p-3 border transition-all duration-300 ease-in-out";
      ...
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-2xl animate-[slideUp_0.6s_ease-out] dark:bg-slate-900 dark:border-slate-700 dark:shadow-black/40">
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="mb-6">
            ...
            >
              <span
                className={`transition-opacity duration-300 ${status === "busy" ? "opacity-0" : "opacity-100"}`}
              >
                {getButtonText()}
              </span>
              {status === "busy" && (
